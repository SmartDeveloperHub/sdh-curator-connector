...
public class AgentParserTest {
	...
	public void testFromModel$happyPath() {
		new ParserTester("data/agent/ok.ttl",FOAF.AGENT_TYPE) {
			@Override
			protected void exercise(Model model, Resource target) {
				Agent agent=AgentParser.fromModel(model, target);
	...
	@Test
	public void testFromModel$fail$no_id() {
		new ParserTester("data/agent/no_id.ttl",FOAF.AGENT_TYPE) {
			@Override
			protected void exercise(Model model, Resource target) {
				Agent agent=AgentParser.fromModel(model, target);
	...
	@Test
	public void testFromModel$fail$bad_type() {
		new ParserTester("data/agent/bad_type.ttl",FOAF.AGENT_TYPE) {
			@Override
			protected void exercise(Model model, Resource target) {
				Agent agent=AgentParser.fromModel(model, target);
				assertThat(agent,nullValue());
	...
	@Test
	public void testFromModel$fail$bad_value() {
		new ParserTester("data/agent/bad_value.ttl",FOAF.AGENT_TYPE) {
			@Override
			protected void exercise(Model model, Resource target) {
				try {
					AgentParser.fromModel(model, target);
					fail("Should not return an agent when bad values are specified");
