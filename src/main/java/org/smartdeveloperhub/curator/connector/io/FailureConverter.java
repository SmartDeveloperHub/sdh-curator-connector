...
final class FailureConverter extends ModelMessageConverter<Failure> {

	private static final String RESPONSE_BNODE = "response";
	private static final String AGENT_BNODE    = "agent";

	@Override
	protected void toString(Failure message, ModelHelper helper) {
		helper.
	...
	@Override
	protected Failure parse(Model model, Resource resource) {
		return FailureParser.fromModel(model, resource);
	}
